<template>
<div class="center"><h2 class="text-center">Bütün Kitaplar</h2></div>
  <Carousel :settings="settings" :breakpoints="breakpoints">
    <Slide v-for="slide in topBook" :key="slide.id">
...
export default {
  ...
  data: () => ({
      ...
      topBook:[],// donen degerı methods bunun ıcıne kaydet
      book: [],
    // carousel settings
    settings: {
      itemsToShow: 1,
  ...
  getid(param)
    ...
    getBooks(){
       BookDataService.getAllBooks()
        .then((response) => {
          this.topBook = response.data;

          console.log(response);
          return JSON.parse(JSON.stringfy(this.topBook));
        })
        .catch((e) => {
          console.log(e);
        });
